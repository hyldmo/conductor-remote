...
import type { SessionPoller } from '../reads/session-poller.ts'
import type { SessionState } from '../reads/types.ts'
import { clipExact, oneLine } from '../voice/speech.ts'
...
async function deliver(device: Device, message: PushMessage): Promise<boolean> {
	...
	// Never log the endpoint: it's a capability URL, and the log is a wire surface (src/host/logbuf.ts).
 ...
 * that `turn_id`; see src/reads/sessions.ts). Empty when the chat records neither, which is what
 ...
 * driven a tick at a time by `tests/notifications/notify.test.ts`. Every rule in it is a rule
