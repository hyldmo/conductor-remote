...
export function WorkflowSummary({
	...
	rootSession?: Pick<Session, 'status'>
	compact?: boolean
}) {
	...
	return (
		...
		>
				...
				<div className="min-w-0 flex-1">
					...
					{workflow.phase === 'blocked' ? (
						<p className="mt-1 text-xs text-text">
							{rootSession?.status === 'working'
								? 'The main chat is still running.'
								: 'Chats already running can continue.'}{' '}
							Workflow steps are waiting for this issue to be resolved.
						</p>
					) : null}
				</div>
			</div>
			...
			<details className="mt-2 text-xs">
				<summary className="w-fit cursor-pointer py-1 text-muted">Workflow details</summary>
			...
			{Object.values(workflow.actions).some(Boolean) ? (
				<details className="mt-2 text-xs" open={workflow.actions.canComplete || undefined}>
					<summary className="w-fit cursor-pointer py-1 text-muted">
						{workflow.phase === 'blocked' ? 'Recovery options' : 'Workflow actions'}
					</summary>
					{workflow.error ? <p className="mt-1 break-words text-[11px] text-faint">{workflow.error.code}</p> : null}
					{workflow.actions.canAdopt && workflow.adoption ? (
						<div className="mt-2 rounded-lg border border-border-soft bg-surface px-2.5 py-2">
							<div className="text-xs font-medium text-text">
								Choose the {workflow.adoption.kind} the relay observed
							</div>
					...
					{confirmReplay && workflow.actions.canReplayAmbiguous ? (
					...
					{confirmCancel && workflow.actions.canCancel ? (
						<div className="mt-2 rounded-lg border border-border-soft bg-surface px-2.5 py-2 text-xs">
							<p className="text-muted">
								Cancel orchestration only. Existing chats, turns, and worktrees stay in place.
							</p>
							<div className="mt-1.5 flex gap-1.5">
								<button
									type="button"
