...
import { ORCHESTRATION_PROTOCOL_VERSION, OrchestrationDb, type ProcessIdentity } from '../src/orchestration-db.ts'
...
describe('orchestration process fault boundary', () => {
	test('concurrent relay startups join the same forward-only migration', async () => {
		...
		const moduleUrl = new URL('../src/orchestration-db.ts', import.meta.url).href
