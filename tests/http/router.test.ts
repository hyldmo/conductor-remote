...
describe('Auto HTTP intake', () => {
	test('creates a uniquely identified workspace and persists Auto before returning', async () => {
		const f = await autoFixture()
		const response = await f.request('/api/workspaces', {
			method: 'POST',
			body: JSON.stringify({ repo: 'example', prompt: 'Move a button.', auto: true, sendImmediately: false })
		})
		expect(response.status).toBe(200)
		expect(f.createWorkspaceAndRead).toHaveBeenCalledExactlyOnceWith('Move a button.', '/unused/repo', 'example', true)
		expect(f.services.autoModels.list()).toMatchObject([
			{ workspaceId: 'workspace-1', sessionId: 'chat-1', text: 'Move a button.', sendImmediately: false }
		])
...
describe('relay HTTP routing', () => {
	...
	test('keeps creation prompt-only, immediate-send and explicit wait choices at the HTTP boundary', async () => {
		...
		expect(first.status).toBe(200)
		expect(f.createWorkspaceAndRead).toHaveBeenLastCalledWith('hello', null, undefined, false)
